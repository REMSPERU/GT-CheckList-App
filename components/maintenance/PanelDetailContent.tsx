    ...
    <View style={styles.container}>
      ...
      <View style={styles.headerSection}>
        <View style={styles.headerTop}>
          <MaterialCommunityIcons name="lightning-bolt-outline" size={24} color="#06B6D4" />
          <Text style={styles.rotuloLabel}>RÓTULO: <Text style={styles.rotuloValue}>{rotulo}</Text></Text>
        </View>
        
        <View style={styles.techGrid}>
          <View style={styles.techItem}>
            <Text style={styles.techLabel}>Montaje</Text>
            <Text style={styles.techValue}>{detalle_tecnico.tipo_tablero}</Text>
          </View>
          <View style={styles.techItem}>
            <Text style={styles.techLabel}>Voltaje</Text>
            <Text style={styles.techValue}>{detalle_tecnico.voltaje} V</Text>
          </View>
          <View style={styles.techItem}>
            <Text style={styles.techLabel}>Fases</Text>
            <Text style={styles.techValue}>{detalle_tecnico.fases}</Text>
          </View>
        </View>
      </View>
          ...
          <View style={styles.itgHeader}>
             <View style={styles.itgTitleRow}>
                ...
                <View>
                    ...
                    {itg.suministra ? <Text style={styles.itgSupply}>Suministra: {itg.suministra}</Text> : null}
                </View>
             </View>
          </View>
            ...
            {itg.itms.map((itm, cIdx) => (
              <View key={cIdx} style={styles.circuitItem}>
                ...
                {itm.diferencial?.existe && (
                  <View style={[styles.differentialBox, { backgroundColor: '#F0F9FF', borderLeftColor: Colors.light.tint }]}>
                    <View style={styles.diffHeader}>
                       <MaterialCommunityIcons name="current-ac" size={14} color={Colors.light.tint} />
                       <Text style={[styles.diffTitle, { color: Colors.light.tint }]}>Diferencial</Text>
                    </View>
                    ...
                    {itm.diferencial.tipo_cable && (
                        <Text style={[styles.diffCable, { color: '#0ea5e9' }]}>
                            {itm.diferencial.tipo_cable} | {itm.diferencial.diametro_cable} mm²
                        </Text>
                    )}
                  </View>
                )}
                  ...
                  <View key={idx} style={styles.componentGroup}>
                      ...
                      {comp.items.map((item, iIdx) => (
                          <View key={iIdx} style={styles.componentRow}>
                              <Ionicons name="hardware-chip-outline" size={16} color="#4B5563" />
               ...
               {Object.entries(condiciones_especiales).map(([key, value]) => (
                   <View 
                    ...
                    style={[
                      ...
                      !value && { backgroundColor: '#F3F4F6', borderColor: '#E5E7EB' }
                    ]}
                   >
                       <Ionicons 
