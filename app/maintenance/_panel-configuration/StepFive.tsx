...
import { StepFiveProps, ExtraComponentType } from './_types';
...
export default function StepFive({
    ...
    return (
        <View style={styles.contentWrapper}>
                    ...
                    {enabledComponents.map(type => {
                        ...
                        return (
                            <View key={type} style={styles.summaryRow}>
                                <Text style={styles.summaryLabel}>{COMPONENT_LABELS[type]}:</Text>
            ...
            {/* Extra Components Detail */}
            {enabledComponents.length > 0 && (
                <View style={{ marginTop: 16 }}>
                    <Text style={[styles.sectionTitle, { marginBottom: 12 }]}>
                        Detalle de Componentes Extras
                    </Text>
                    {enabledComponents.map(type => {
                        const components = extraComponents[type];
                        if (components.length === 0) return null;
                        ...
                        return (
                            <View key={`detail-${type}`} style={{ marginBottom: 12 }}>
                                <Text style={[styles.cnSectionTitle, { marginBottom: 8 }]}>
                                    {COMPONENT_LABELS[type]}
                                </Text>
                                {components.map((component, idx) => (
                                    <View key={`${type}-${idx}`} style={styles.summaryRow}>
                                        <Text style={styles.summaryLabel}>{component.id}:</Text>
                                        <Text style={styles.summaryValue}>
