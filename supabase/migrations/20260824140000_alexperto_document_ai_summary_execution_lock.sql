...
BEGIN
  ...
  SELECT summary.status, summary.execution_id
  INTO v_status, v_current_execution_id
  FROM public.alexperto_document_ai_summaries AS summary
  WHERE summary.request_id = p_request_id
    AND summary.document_id = p_document_id
    AND summary.document_hash = p_document_hash
  FOR UPDATE;
