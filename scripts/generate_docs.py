...
El servicio [sync.ts](../services/sync.ts) realiza la orquestación periódica del flujo de datos en segundo plano mediante las siguientes reglas:
...
* **Vistas Relacionadas:** [forgot-password](../app/auth/forgot-password) / [login](../app/auth/login) / [register](../app/auth/register).
...
* **Vistas Relacionadas:** [systems.tsx](../app/inventory/systems.tsx) / [add-equipo.tsx](../app/inventory/[equipamentoId]/add-equipo.tsx) / [detail.tsx](../app/inventory/[equipoId]/detail.tsx).

### C. Escáner de Códigos QR
* Usando la cámara integrada del dispositivo, el módulo [qr-scanner.tsx](../app/(tabs)/qr-scanner.tsx) lee el código QR adherido al equipo físico.
...
* **Vistas Relacionadas:** [maintenance/index.tsx](../app/maintenance/index.tsx) / [checklist/form.tsx](../app/checklist/form.tsx).
...
* **Vistas Relacionadas:** [electrical-panels/configuration.tsx](../app/maintenance/electrical-panels/configuration.tsx) y los pasos contenidos en [_config-steps/](../app/maintenance/electrical-panels/_config-steps/).
...
* **Vista Relacionada:** [grounding-well/checklist.tsx](../app/maintenance/execution/grounding-well/checklist.tsx).
...
* **Vistas Relacionadas:** [auditoria/index.tsx](../app/auditoria/index.tsx) / [auditoria/session.tsx](../app/auditoria/session.tsx).

### H. Generador de Reportes PDF Local
* Consumiendo los datos técnicos estructurados guardados localmente, el servicio [pdf-report/service.ts](../services/pdf-report/service.ts) ensambla plantillas HTML con estilos premium y paletas de colores corporativas.
...
* **Vistas Relacionadas:** [usuarios/page.tsx](../web/app/admin/usuarios/page.tsx).
...
* **Vistas Relacionadas:** [equipos/qr/page.tsx](../web/app/admin/equipos/qr/page.tsx).
...
* **Vistas Relacionadas:** [mantenimientos/page.tsx](../web/app/admin/mantenimientos/page.tsx).
...
* **Vistas Relacionadas:** [checklist/page.tsx](../web/app/admin/checklist/page.tsx).
...
* **Vistas Relacionadas:** [auditorias/page.tsx](../web/app/admin/auditorias/page.tsx) / [auditorias/[auditId]/page.tsx](../web/app/admin/auditorias/[auditId]/page.tsx).
...
```mermaid
sequenceDiagram
    participant Tec as Técnico (App Móvil)
    participant SQLite as SQLite Local
    participant Sync as SyncService
    participant Storage as Supabase Storage
    participant DB as Supabase DB (Nube)
    participant Admin as Admin (Panel Web)

    Tec->>SQLite: Guarda checklist respondido + fotos locales
    Note over Tec, SQLite: Operación offline garantizada
    
    Sync->>SQLite: Consulta cola de pendientes (offline_*)
    activate Sync
    Sync->>Storage: Sube fotos locales de evidencia
    Storage-->>Sync: Retorna URLs públicas de imágenes
    Sync->>DB: Inserta datos estructurados (payload + URLs)
    DB-->>Sync: Confirmación de guardado
    Sync->>SQLite: Actualiza registros a 'synced' y limpia cola
    deactivate Sync

    Admin->>DB: Consulta reportes / Modifica estados
    Admin->>Storage: Carga/Elimina fotos adicionales
```
